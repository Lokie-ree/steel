...
- **Next: nothing in code.** Two operator-only items block the gate — the school-network check and the collection protocol (see This week's focus). Then run the module with students and read it against [[../wiki/depth-criteria]].
...
9. **Collection protocol + one DEMO01 dry run** — [[../wiki/depth-criteria]] §OPEN. Write where the CSVs land, who triggers the export, what happens on a storage-blocked device, and how ~40 devices' files become one file; then run it end to end with `DEMO01`, on a real school device if the network check is happening anyway. Without this, the first class period produces no readable data and the build-two gate cannot open no matter how well the module performs.
