...
**2026-09-22: receipts from outside the vault.** The operator brought back four system summaries, each written by the session that runs one project: [[../projects/loop-bench]], [[../projects/algebra2-course-plan]], [[../projects/civics-planning-tool]], and the Tech Station ([[issue-ledger]]). These are AI self-reports. Nothing in them is verified until it's opened ([[../operator/build-inventory]] rules).

- **Four sessions on two accounts describe the same skeleton,** and none copied another: a slow-changing base layer, a per-unit pipeline, a check that fails loudly, and a handoff or ruling record. The exception is the lesson-planning tool. Its layers are sorted by how often they change, it has no check layer, and it's the only one blocked on another person's answers.
- **The reading test ran, and it failed at the account boundary.** The Tech Station's session called *derive, don't transcribe* "waiting to be written into steel." Steel already holds it ([[../CLAUDE]] §Git state; [[../wiki/lessons]] #2–3; [[issue-ledger]] records the independent convergence). That session runs on the district account and can't read the vault. **So the proposed open step can't be "read `wiki/lessons.md`."** Lessons have to travel in a form both accounts can load. The lesson-planning tool's session proposed the same thing from its side: package the context as a skill. The Algebra II skill already works this way.
- **Lesson #2 gets evidence from the classroom side.** It's the same failure class three times: the loop-bench transfer function before a second file copies it, the stale Algebra II skill and its helper script, and the Tech Station's count drift. That's the first support for "lessons cross both ways" that doesn't come from the hackathon.
- **One report named a file that doesn't exist.** The Tech Station's session said steel holds an `ipsb-teacher-tech-station.md` card. There is no such card. Steel tracks it in [[issue-ledger]]. This is lesson #3, and it applies to this session too: the first check here missed the ledger because it searched a hyphenated spelling.
- **Two open questions carry across projects:** whether licensed publisher materials may be uploaded to an external tool (already open on [[../projects/civics-planning-tool]]), and whether cleaned district documents replace the originals or stay a coaching aid. Both apply to Algebra II on EdgeEx as well.
- **What this suggests the vault is:** the only place that sees across both accounts. Its job is a pointer for every live project, plus lessons in a form that crosses the account boundary. Syncing repos is not the job. Two of the four live projects have no repo.
