...
> **Window closed 2026-08-10** — the first day for students, which was this sprint's own
> deadline. This doc is historical; read the priority stack below as the record of the
> runway, not as current work.
>
> **Two items were still live when it closed** and carry forward to
> [[../sprint/2026-first-contact]]: **8b** (which network the room actually runs on) and
> **9b** (the `DEMO01` dry run that 8b gates). Both are operator-only, neither is code, and
> both were open for the full thirteen days the window sat expired.
...
**Assignment correction (2026-07-22):** the 2026–27 room is **two preps** — Algebra II and Algebra III on EdgeEx — not six. The infrastructure already built was sized for the larger workload, so the shrink is slack: fewer, deeper builds, gated on real student data ([[../wiki/decisions]] D-2026-07-22b).
...
| — | **creative-lab** | **Live spoke, no fall build** — by ruling, not neglect ([[../wiki/decisions]] D-2026-07-22a). R3F/CSG stays a shipping capability; neither EdgeEx course has spatial content to aim it at. |
...
| 2026-08-03 | steel | Strategy session held **outside the vault** (mentor chat, no repo state changed). Build surface narrowed to **Algebra II, 50-minute block, one existing module, one door** — entered with "the entire math spectrum." Three rulings ratified ([[../wiki/decisions]] D-2026-08-03a, D-2026-08-03b, D-2026-08-03c); the second-door prescription held pending the load-bearing question. The geometry ruling was reopened on a verbal claim and re-closed by the v2 schedule inside one turn — **an artifact closed it, a sentence did not** |
...
| 2026-08-13 | steel | **Two non-spoke cards opened** — [[../projects/civics-planning-tool]] (Iberville lesson-planning tool, one pilot teacher) and [[../projects/algebra2-course-plan]] (Algebra II Sem A, which had been living only in a Claude project and a skill, with no vault pointer at all). `projects/index.md` gains a **Not spokes** section; its header line had asserted every card in the folder was a repo. Neither card introduces a `sync-registry` field or anything for `drift-check`. **Founding them was a ratified decision, not a default** — the session opened by searching the vault for an existing Iberville lesson-planning note, finding none, and stopping rather than founding one. [[../initiatives/issue-ledger]] was the near-miss and was rejected on its own rule (*steel holds the pointer, never the items*), the same rule that discarded the chat-authored IPSB card on 2026-08-03 |
| 2026-08-13 | steel | Planning-tool card substance: the **LDOE HS scoring notes** for the subject land in the context stack's Calibration layer as a **Column 2 source, superseding reverse-engineering success criteria from textbook quiz items** — 5 units, 13 topics, each with a framing question, a constructed-response prompt, and LDOE's own exemplar content. Closest thing yet to what *must-do* means in Column 5. **Crosswalk seam:** the district template header names an LDOE unit title while the week's content follows the textbook's chapter numbering — the district is already planning in both maps at once, which is the evidence for flipping the source anchor to standards-centered (**held as needing ratification, with a source field on the weekly intake block — neither recorded as decided**). Three standing tool requirements: code substandards **per day** (one validation week spanned C.8a, C.8b, C.11.a under a single code), use **the standard's** typology in Column 1 with the textbook's set demoted to vocabulary, and **flag under-rigor rather than smooth it** ("define"/"identify" objectives against "compare and contrast"/"analyze" standards). All four are written as requirements on the tool, never as an assessment of a teacher's plans — the framing is the rule, and it is stated inside the card so it outlives the session that wrote it. Seven items left open, none resolved |
